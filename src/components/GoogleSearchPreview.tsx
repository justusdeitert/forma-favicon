...
export const GoogleSearchPreview = ({ faviconUrl, cacheBuster, livePreviewUrl, dark = false }: Props) => {
	...
	const displayUrl = siteUrl ? siteUrl.replace(/^https?:\/\//, '').replace(/\/$/, '') : 'example.com';

	return (
		<div className="mb-8">
			<h2 className="text-base font-medium text-gray-700 m-0 mb-1">Google Search</h2>
			<p className="text-xs text-gray-400 m-0 mb-4">How your favicon appears in Google search results.</p>
			...
			>
				...
				<div className="flex items-center gap-3 mb-1">
					...
					<div className="flex flex-col min-w-0">
						<span className={`text-sm leading-tight truncate ${dark ? 'text-[#bdc1c6]' : 'text-[#202124]'}`}>
							{displayUrl}
						</span>
						<span className={`text-xs leading-tight truncate ${dark ? 'text-[#969ba1]' : 'text-[#4d5156]'}`}>
				...
				<p className="m-0 text-sm leading-relaxed" style={{ color: dark ? '#bdc1c6' : '#4d5156' }}>
